    ...
    try:
        while True:
            ...
            for key in expired_keys:
                ...
                cancel_timeout_task(key) # استفاده از تابع ایمن
...
# تابع کمکی برای ایجاد متن لیست بازیکنان (نسخه اصلاح شده)
def get_players_text(session):
    ...
    for p in sorted_players:
        # متن امتیاز را فقط در صورتی که بازی شروع شده باشد، اضافه می‌کنیم
...
async def start_command_handler(event):
    ...
    # قبل از ایجاد جلسه جدید، هر تایمر قدیمی مربوط به این چت را لغو کن
...
# ##################################################################
# ############## مهم‌ترین تغییر برای رفع مشکل شما اینجاست #############
# ##################################################################
async def ask_question_in_chat(client, session_key):
    ...
    # --- بخش کلیدی برای حل مشکل ---
    # قبل از ایجاد تایمر جدید، هر تایمر قدیمی را برای این جلسه لغو و حذف می‌کنیم
    cancel_timeout_task(session_key)
    ...
    # اکنون با خیال راحت تایمر جدید را ایجاد می‌کنیم
...
async def question_timeout(client, session_key):
    try:
        ...
        # نیازی به raise نیست چون لغو شدن در این منطق طبیعی است
    except Exception as e:
